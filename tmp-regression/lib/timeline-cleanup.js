...
exports.cleanTimelineEvents = cleanTimelineEvents;
exports.rankTimelineEvents = rankTimelineEvents;
exports.filterTimelineForDisplay = filterTimelineForDisplay;
...
function isGarbageProviderName(value) {
...
function normalizeClinicianName(value) {
    ...
    if (normalized === "physician")
        return null;
    if (normalized.includes("olivia king") ||
    ...
    if (/^[a-z]{1,3}$/.test(normalized))
        return null;
    if (isGarbageProviderName(raw))
        return null;
    if (normalized.includes("sudolcan"))
        return "Dr. Sudolcan";
    if (normalized.includes("sarah orrin"))
        return "Sarah Orrin MD";
    if (normalized.includes("ethel king") || normalized.includes("king ethel")) {
        return "Ethel King";
    }
    if (normalized.includes("vretis"))
        return "Dr. Vretis";
...
function cleanProviderRole(value) {
    ...
    if (raw.includes("physician"))
        return "Physician";
    if (raw.includes("radiolog"))
        return "Radiologist";
    if (raw.includes("ordering"))
        return "Ordering Provider";
    if (raw.includes("attending"))
        return "Attending Physician";
    return titleCaseName(raw);
}
function getEventText(event) {
    return normalizeText(`${event.title || ""} ${event.description || ""} ${event.sourceExcerpt || ""} ${event.providerName || ""} ${event.physicianName || ""}`);
}
function getEventSupportText(event) {
    return normalizeText(`${event.title || ""} ${event.description || ""} ${event.sourceExcerpt || ""}`);
}
function isImagingEventText(text) {
    return /\b(ct|x ray|xray|scan|imaging|radiology|exam|study)\b/.test(text);
}
function isErPhysicianNoteEventText(text) {
    return /\b(emergency physician|er physician|ed physician|provider note|physician note|electronically signed)\b/.test(text);
}
function isTransferAcceptanceEventText(text) {
    return /\b(transfer|accepted|accepting|air transport|higher level of care)\b/.test(text) && /\bshannon\b/.test(text);
}
function shouldAssignPhysician(event) {
    const supportText = getEventSupportText(event);
    if (supportText.includes("electronically signed"))
        return true;
    if (isErPhysicianNoteEventText(supportText) &&
        /\b(chief complaint|history of present illness|physical exam|clinical impression|assessment)\b/.test(supportText)) {
        return true;
    }
    if (supportText.includes("accepting") &&
...
function hasExplicitClinicianSupport(event, clinicianName) {
    if (!clinicianName)
        return false;
    ...
    return (!!lastToken &&
        ...
        /\b(signed|electronically signed|accepting|attending|rendering|author|radiologist|physician|provider|fnp)\b/.test(supportText));
}
function hasExplicitFacilitySupport(event, facility) {
    if (!facility)
        return false;
    const supportText = getSupportText(event);
    if (!supportText)
        return false;
    const tokens = getMeaningfulNameTokens(facility).filter((token) => ![
        "hospital",
        "medical",
        "center",
        "clinic",
...
function looksAdministrativeOnly(text) {
...
function hasMeaningfulTraumaFinding(text) {
    const normalized = normalizeText(text);
    if (/\bscalp laceration\b/.test(normalized))
        return true;
    if (/\bhead\b/.test(normalized) && /\blaceration\b/.test(normalized))
        return true;
    if (/\bleft eye\b/.test(normalized) &&
...
function hasMeaningfulMechanismOfInjury(text) {
    const normalized = normalizeText(text);
    return (/\b(pipe|derrick|drill rig)\b/.test(normalized) &&
        /\b(fell|fall|hit|injury|trauma)\b/.test(normalized) &&
        /\bhead\b/.test(normalized));
}
function getConceptKey(event) {
    ...
    if (/\borders? discontinued\b/.test(combined) &&
    ...
    if (/\b(no blasts|blasts not seen|no blast seen|no blasts seen)\b/.test(combined)) {
        return "smear_no_blasts";
    }
    if (/\b(platelet estimate markedly decreased|marked platelet decrease|platelets markedly decreased)\b/.test(combined)) {
        return "smear_platelet_decrease";
    }
    if (/\banisopoikilocytosis\b/.test(combined)) {
        return "smear_anisopoikilocytosis";
    }
    if (/\b(reflex microscopy and urine culture triggered|urinalysis triggered reflex microscopy|urinalysis triggered urine culture|reflex testing was initiated|reflexively ordered)\b/.test(combined)) {
        return "ua_reflex_workflow";
    }
    if (/\b(laboratory encounter opened|laboratory encounter recorded|comprehensive laboratory encounter)\b/.test(combined)) {
        return "lab_encounter_admin";
    }
    if (/\brespiratory viral pcr collected\b/.test(combined) ||
    ...
    if (/\b(respiratory viral pcr negative|not detected)\b/.test(combined) &&
    ...
    if (/\b(telemetry|hospitalization|hospitalisation|admit)\b/.test(combined) &&
        /\bangina\b/.test(combined)) {
        return "plan_hospitalization";
    }
    return "";
}
function getLabPanelType(text) {
    if (/\b(cbc|complete blood count|wbc|hemoglobin|hematocrit|rbc|anc|absolute neutrophil|absolute lymphocyte|platelet count|platelet estimate|platelets?)\b/.test(text) &&
        /\b(lab|result|results|blood|count|smear|differential|hematology|panel|level|x10\^3|x10\^6)\b/.test(text)) {
        return "cbc";
    }
    if (/\b(cmp|bmp|metabolic panel|creatinine|bun|sodium|potassium|chloride|co2|ast|alt|alk phos|bilirubin)\b/.test(text) &&
...
function isStandaloneNormalPanelLabEvent(event) {
    const title = normalizeText(cleanTitle(event.title));
    const description = normalizeText(cleanDescription(event.description));
    const combined = `${title} ${description}`.trim();
    const genericPanelTitle = /^(cbc|complete blood count|bmp|basic metabolic panel|cmp|comprehensive metabolic panel|metabolic panel)$/.test(title) || /^(cbc|bmp|cmp)\b/.test(title);
    if (!genericPanelTitle)
        return false;
    const abnormalOrMeaningfulPatterns = [
    ...
    const plainNumericPanel = /\b(wbc|hgb|hemoglobin|hematocrit|rbc|platelets?|sodium|potassium|chloride|co2|creatinine|bun|glucose|calcium)\b/.test(description) && /\d/.test(description);
    return plainNumericPanel;
}
function getImagingModality(text) {
    if (/\bct\b/.test(text))
        return "ct";
    if (/\bx ray\b|\bxray\b/.test(text))
        return "xray";
    if (/\bmri\b/.test(text))
        return "mri";
    if (/\bultrasound\b/.test(text))
        return "us";
    return "";
}
function getImagingRegion(text) {
    if (/\bc2\b|\bcervical\b|\bneck\b/.test(text))
        return "cervical";
    if (/\bscapula(?:r)?\b|\bshoulder\b/.test(text))
        return "scapula";
    if (/\b(periorbital|orbital|orbit|eye|facial|face)\b/.test(text))
        return "orbit-face";
    if (/\bhead\b|\bscalp\b/.test(text))
        return "head";
    if (/\bchest\b/.test(text))
        return "chest";
    if (/\babdomen\b|\bpelvis\b/.test(text))
...
function getMergeGroupKey(event) {
    ...
    if (/\b(urinalysis|urine)\b/.test(text) &&
...
function getDistinctFragments(events) {
    const fragments = [];
...
function buildGroupedLabTitle(events, fallbackTitle) {
    const combined = normalizeText(events
    ...
    if (!panelType || findings.length === 0) {
        if (panelType === "urinalysis") {
            const urineFindings = [];
            if (/\bnitrite\b/.test(combined))
                urineFindings.push("positive nitrite");
            if (/\bleukocyte esterase\b/.test(combined))
                urineFindings.push("positive leukocyte esterase");
            if (/\bprotein\b/.test(combined))
                urineFindings.push("proteinuria");
            if (/\bglucose\b/.test(combined))
                urineFindings.push("glucosuria");
            if (/\bspecific gravity\b/.test(combined))
                urineFindings.push("concentrated urine");
    ...
    const panelLabel = panelType === "cbc"
        ? "CBC showed"
        : panelType === "chemistry"
...
function buildGroupedLabDescription(events, fallbackDescription) {
    const combined = normalizeText(events
        .map((event) => `${event.title || ""} ${event.description || ""}`)
        .join(" "));
    const panelType = getLabPanelType(combined);
    const notableFragments = [];
    const fragmentMap = [
        [/\bleukocytosis\b/, "leukocytosis"],
        [/\bleukopenia\b/, "leukopenia"],
        [/\bthrombocytopenia\b|\bplatelets? (?:low|decreased|markedly decreased)\b/, "thrombocytopenia"],
...
function buildGroupedImagingTitle(events, fallbackTitle) {
    const combined = normalizeText(events
        .map((event) => `${event.title || ""} ${event.description || ""}`)
        .join(" "));
    const modality = getImagingModality(combined);
    const modalityLabel = modality === "ct"
        ? "CT"
        : modality === "xray"
            ? "X-ray"
            : modality === "mri"
...
function buildGroupedImagingDescription(events, fallbackDescription) {
    const combined = normalizeText(events
...
function buildGroupedIncidentTitle(events, fallbackTitle) {
    const combined = normalizeText(events
        .map((event) => `${event.title || ""} ${event.description || ""}`)
        .join(" "));
    ...
    if (/\bclinical impression\b/.test(combined) &&
...
function buildGroupedIncidentDescription(events, fallbackDescription) {
    const combined = normalizeText(events
        .map((event) => `${event.title || ""} ${event.description || ""} ${event.sourceExcerpt || ""}`)
        .join(" "));
    if (hasMeaningfulMechanismOfInjury(combined)) {
...
function buildGroupedTraumaExamTitle(events, fallbackTitle) {
    const combined = normalizeText(events
        .map((event) => `${event.title || ""} ${event.description || ""}`)
        .join(" "));
    if (/\b(head|scalp)\b/.test(combined) &&
    ...
    if (/\b(periorbital|orbital|left eye|left orbit)\b/.test(combined) &&
        /\b(bruising|bruised|swelling|swollen|ecchymosis)\b/.test(combined)) {
        return "Left periorbital swelling and bruising documented";
    }
    return fallbackTitle;
}
function buildGroupedTraumaExamDescription(events, fallbackDescription) {
    const combined = normalizeText(events
        .map((event) => `${event.title || ""} ${event.description || ""} ${event.sourceExcerpt || ""}`)
        .join(" "));
    const details = [];
    ...
    if (/\bleft eye\b/.test(combined) &&
        /\b(bruising|bruised|swelling|swollen|ecchymosis)\b/.test(combined)) {
        details.push("The left eye was bruised and swollen shut.");
    }
    else if (/\bleft\b/.test(combined) &&
...
function mergeGroupedEvents(events) {
    ...
    const merged = Array.from(grouped.values()).map((group) => {
        if (group.length === 1)
            return group[0];
        const base = group.reduce((best, current) => chooseBetterEvent(best, current));
        const key = getMergeGroupKey(base);
        const fragments = getDistinctFragments(group).filter((fragment) => normalizeText(fragment) !== normalizeText(base.title));
        const mergedDescription = fragments.length > 0 ? fragments.slice(0, 4).join("; ") : cleanDescription(base.description);
        let mergedTitle = cleanTitle(base.title);
        let finalDescription = mergedDescription || cleanDescription(base.description) || null;
        if (key.includes("|lab")) {
            mergedTitle = buildGroupedLabTitle(group, mergedTitle);
            finalDescription = buildGroupedLabDescription(group, finalDescription || "");
        }
        if (key.includes("|imaging:")) {
            mergedTitle = buildGroupedImagingTitle(group, mergedTitle);
            finalDescription = buildGroupedImagingDescription(group, finalDescription || "");
        }
        if (key.includes("|incident:mechanism")) {
            mergedTitle = buildGroupedIncidentTitle(group, mergedTitle);
            finalDescription = buildGroupedIncidentDescription(group, finalDescription || "");
        }
        if (key.includes("|exam:trauma")) {
            mergedTitle = buildGroupedTraumaExamTitle(group, mergedTitle);
            finalDescription = buildGroupedTraumaExamDescription(group, finalDescription || "");
        }
        return {
            ...base,
            title: mergedTitle,
...
function isBoilerplateTitle(title) {
    ...
    if (exactBadTitles.has(normalized))
        return true;
    const badPatterns = [
        /\bappears well\b/i,
...
function shouldDropLowSignalEvent(event) {
    ...
    if (conceptKey === "admin_referral_note" ||
    ...
    if (lowSignalPatterns.some((pattern) => pattern.test(combined)) &&
        !hasClinicalContent) {
        return true;
    }
    if ((conceptKey === "lab_encounter_admin" ||
...
function hasClinicalValue(text) {
    const normalized = normalizeText(text);
    ...
    return (clinicalPatterns.some((pattern) => pattern.test(normalized)) ||
        hasMeaningfulTraumaFinding(normalized));
}
function areLikelyDuplicateEvents(a, b) {
    ...
    if (hasConflictingLaterality(aText, bText))
        return false;
    if (/\bunstable angina\b/.test(aText) !== /\bunstable angina\b/.test(bText)) {
        return false;
    }
    if (aType !== bType &&
...
function chooseBetterEvent(a, b) {
    ...
    if (aConceptKey && bConceptKey && aConceptKey === bConceptKey) {
        ...
        if (aConfidence > bConfidence)
            return a;
        if (bDescription.length > aDescription.length)
            return b;
        if (aDescription.length > bDescription.length)
            return a;
        if (bTitle.length > aTitle.length)
            return b;
        return a;
    }
    if (bSpecificity > aSpecificity)
...
function dedupeEvents(events) {
    const deduped = [];
    for (const event of events) {
        const existingIndex = deduped.findIndex((existing) => areLikelyDuplicateEvents(existing, event));
        if (existingIndex === -1) {
            deduped.push(event);
        }
        else {
...
function improveTitle(event) {
    ...
    if (/\bvascular injury\b/.test(combined) ||
    ...
    if (/\burinalysis\b/.test(combined) &&
    ...
    if (/\bleft\b/.test(combined) &&
        /\b(periorbital|orbital)\b/.test(combined) &&
        /\b(bruising|bruised|swelling|swollen|ecchymosis)\b/.test(combined)) {
        return "Left periorbital swelling and bruising documented";
    }
    if (/\bsmear\b/.test(combined) &&
...
function normalizeEvents(events) {
    return events.map((event) => {
        ...
        let cleanedPhysicianNameRaw = normalizeClinicianName(event.physicianName) || cleanedProviderName;
        let cleanedPhysicianRole = cleanProviderRole(event.physicianRole) || cleanedProviderRole;
        ...
        return {
            ...
            sourcePage: typeof event.sourcePage === "number" ? event.sourcePage : null,
...
function scoreEvent(event) {
    const text = normalizeText(`${event.title || ""} ${event.description || ""}`);
    let score = 0;
    if (/fracture|hemorrhage|injury|trauma|vascular injury/.test(text))
        score += 60;
    if (/transfer|accepted|air transport|higher level of care/.test(text))
        score += 45;
    if (/impression|diagnosis/.test(text))
        score += 35;
    if (/ct|x ray|imaging|radiology/.test(text))
        score += 25;
    if (/laceration|swelling|disc herniation|scapular|c2/.test(text))
        score += 20;
    if (/pipe|derrick|drill rig|periorbital|orbit/.test(text))
        score += 18;
    if (/iv|oxygen|monitor|nursing|pain score|transported by ems|ems/.test(text))
        score -= 40;
    if (/cbc|cmp|metabolic panel|lab/.test(text))
        score -= 15;
    if (/social history|past medical|past surgical|smoker|drinker/.test(text))
        score -= 35;
    if (/report called|awaiting|resting|educated|returned from x ray/.test(text))
        score -= 35;
    return score;
}
...
function hasConflictingLaterality(aText, bText) {
...
function getHighLevelDuplicateKey(event) {
    const date = normalizeDate(event.date);
    const text = normalizeText(`${event.title || ""} ${event.description || ""}`);
    const laterality = getLaterality(text);
    if (/\bclinical impression\b/.test(text) &&
        /\btrauma\b/.test(text) &&
        /\bc2\b/.test(text) &&
    ...
    if (/\b(ct|x ray|xray|exam|scan|report|study|imaging|radiology)\b/.test(text) &&
        /\bscapula(?:r)?\b/.test(text) &&
        /\bfracture\b/.test(text)) {
        return `${date}|scapula-imaging|${laterality || "unknown"}`;
    }
    if (/\b(transfer|accepted|accepting)\b/.test(text) &&
...
function getSpecificityScore(event) {
    const text = normalizeText(`${event.title || ""} ${event.description || ""}`);
    let score = 0;
    if (cleanDescription(event.description))
        score += 2;
    if (/clinical impression/.test(text))
        score += 3;
    if (/\btrauma\b/.test(text))
        score += 2;
    if (/\bc2\b/.test(text) && /\bfracture\b/.test(text))
        score += 3;
    if (/\bscapula(?:r)?\b/.test(text) && /\bfracture\b/.test(text))
        score += 3;
    if (/\bleft\b|\bright\b|\bbilateral\b/.test(text))
        score += 2;
    if (/\btransfer\b/.test(text) && /\bshannon\b/.test(text))
        score += 3;
    if (/\baccepted|accepting|higher level of care|air transport\b/.test(text))
        score += 2;
    if (hasMeaningfulTraumaFinding(text))
        score += 3;
    return score;
}
function areHighLevelClinicalDuplicates(a, b) {
    const aText = normalizeText(`${a.title || ""} ${a.description || ""}`);
    const bText = normalizeText(`${b.title || ""} ${b.description || ""}`);
    if (normalizeDate(a.date) !== normalizeDate(b.date))
        return false;
    if (hasConflictingLaterality(aText, bText))
        return false;
...
function dedupeAndRankEvents(events) {
    const map = new Map();
    for (const event of events) {
    ...
    for (const event of pass1) {
        const existingIndex = deduped.findIndex((existing) => areHighLevelClinicalDuplicates(existing, event));
        if (existingIndex === -1) {
            deduped.push(event);
        }
        else {
            const currentScore = ((deduped[existingIndex]._score ?? 0) + getSpecificityScore(deduped[existingIndex]));
            const nextScore = ((event._score ?? 0) + getSpecificityScore(event));
            if (nextScore > currentScore) {
                deduped[existingIndex] = event;
            }
        }
    }
    return deduped.sort((a, b) => ((b._score ?? 0) -
        (a._score ?? 0)));
}
...
function rankTimelineEvents(events, limit = 15) {
    return dedupeAndRankEvents(events).slice(0, limit);
}
const DISPLAY_HIDE_TITLE_PATTERNS = [
    /\blaboratory report finalized\b/i,
    /\breport finalized\b/i,
...
const DISPLAY_HIDE_DESCRIPTION_PATTERNS = [
...
const MEANINGFUL_NEGATIVE_PATTERNS = [
    /\bnegative\b/i,
    /\bnot detected\b/i,
    /\bno evidence of\b/i,
    /\bno blasts?\b/i,
];
const IMPORTANT_NEGATIVE_CONTEXT_PATTERNS = [
    /\bpcr\b/i,
    /\bviral\b/i,
    /\binfluenza\b/i,
    /\brsv\b/i,
...
function isLowValueEmsWorkflowEvent(event) {
...
function isAdministrativeDisplayEvent(event) {
    const title = cleanTitle(event.title);
    const description = cleanDescription(event.description);
    const conceptKey = getConceptKey(event);
    const combined = `${title} ${description}`.trim();
    ...
    if (matchesAnyPattern(title, DISPLAY_HIDE_TITLE_PATTERNS) &&
        !hasClinicalValue(combined)) {
        return true;
    }
    if (matchesAnyPattern(description, DISPLAY_HIDE_DESCRIPTION_PATTERNS) &&
        !hasClinicalValue(combined)) {
        return true;
    }
    return false;
}
function isLowValueNegativeEvent(event) {
    const title = cleanTitle(event.title);
    const description = cleanDescription(event.description);
    const combined = `${title} ${description}`.trim();
    const hasNegativeLanguage = matchesAnyPattern(combined, MEANINGFUL_NEGATIVE_PATTERNS);
    const hasImportantContext = matchesAnyPattern(combined, IMPORTANT_NEGATIVE_CONTEXT_PATTERNS);
    return hasNegativeLanguage && !hasImportantContext;
}
function isGenericProcedureEvent(event) {
    const title = normalizeText(event.title);
    const desc = normalizeText(event.description);
    const combined = `${title} ${desc}`.trim();
    if (hasClinicalValue(combined))
        return false;
    const hasMeaningfulProcedureContext = /\b(ct|x ray|xray|mri|ultrasound|ekg|ecg|imaging|radiology)\b/.test(combined) &&
    ...
    if (hasMeaningfulProcedureContext)
        return false;
    ...
    if (weakProcedurePatterns.some((p) => p.test(title) || p.test(desc)) &&
        !hasClinicalValue(combined)) {
        return true;
    }
    return false;
}
function filterTimelineForDisplay(events) {
    return events.filter((event) => {
        ...
        if (isLowValueEmsWorkflowEvent(event) &&
        ...
        if (/pain score|iv line|oxygen|report called|resting|awaiting/.test(combined) &&
            !hasClinicalValue(combined)) {
            return false;
        }
        if (!physicianName &&
            providerName &&
            /rn|ems|nurse|emt/.test(normalizeText(`${event.providerRole || ""} ${providerName}`))) {
            return false;
        }
        return true;
    });
